...
dir.create(saveDir0)
round <- "round1"
saveDir1 <- paste0(saveDir0, round, "/plots/FOIBootstrap/")
dir.create(saveDir1)
          ...
          for (m in 1:length(labels)) {
            ...
            for (n in 1:length(MOI_states)) {
              ...
              for (p in 1:length(methods)) {
                method <- methods[p]
                readDirFinal <- paste0(readDir, run, "/", type, "/FOIBootstrap/", method, "/", s, "/", o, "/", 
                                       MOI_state, "/", prefix, "_", num, "/rep_", rep, "/", ls, "/")
                if (method == "twoMoments") {
                  fileNames <- paste0("bootstrap_rep_", bs_reps, "-", bs_reps, ".RData")
                  FOI_est_bootstrap_all <- NULL
                  for (q in 1:length(fileNames)) {
                    fileName <- fileNames[q]
                    load(paste0(readDirFinal, fileName))
                    temp0 <- FOI_est_bootstrap %>%
                      arrange(likelihood) %>%
                      slice(seq_len(1))
                    FOI_est_bootstrap_all <- rbind(FOI_est_bootstrap_all, temp0)
                  }
                  temp1 <- data.frame("FOI_mean_est" = T_YEAR/FOI_est_bootstrap_all$meanInterarrival,
                                      "varInterarrival_est" = FOI_est_bootstrap_all$VarInterArrival,
                                      "MOI_state" = MOI_state,
                  ...
                  load(paste0(readDirFinal, fileName))
                  temp1 <- data.frame("FOI_mean_est" = FOI_est_bootstrap$FOI,
            ...
            p1<-ggplot(FOIdf, aes(x=IRSPhase, y = FOI_mean_est, fill = method))+
