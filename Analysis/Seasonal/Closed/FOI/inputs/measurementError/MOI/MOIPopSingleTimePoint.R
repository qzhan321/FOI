...
saveDir0 <- paste0("/project2/pascualmm/QZ/PhD/projects/FOI/", folder, "/round", saveRoundIndex, "/files/", arrivalType, "/FOI/inputs/MOIPopSingleTimePoint/")
      ...
      for (k in 1:length(layers)) {
        ...
        for (nRealization in 1:nRealizations) {
          ...
          sampled_hosts_sub <- left_join(sampled_hosts, hosts, by = c("host_id")) %>% 
            ...
            select(host_id) %>% mutate(DBLa_upsBC_rep_size = 0, MOI = 0, Prob = 1)
          stopifnot(all(unique(MOIMea$HostID) %in% unique(sampled_hosts_sub$host_id)))
          hostsNoInf <- setdiff(unique(sampled_hosts_sub$host_id), unique(MOIMea$HostID))
          sampled_hosts_sub <- sampled_hosts_sub %>% filter(host_id %in% hostsNoInf)
          colnames(sampled_hosts_sub)[1] <- "HostID"
          
          MOI <- do.call("bind_rows", list(MOIMea, sampled_hosts_sub)) %>% mutate(time = layer, nRealization = nRealization)
          MOIAll <- rbind(MOIAll, MOI)
        }
        MOIAllFinal <- MOIAll %>% group_by(MOI) %>% summarise(n = sum(Prob)) %>% ungroup() %>% 
          mutate(n_total = sum(n), Prob = n/n_total, N = n_total/nRealizations) %>% select(-c(n, n_total))
        write.csv(MOIAllFinal, file = paste0(saveDir4, "r", r, "_", layerLabel, "_", ageGroupLabel, ".csv"), row.names = F)
      }
    }
    dbDisconnect(db)
  }
