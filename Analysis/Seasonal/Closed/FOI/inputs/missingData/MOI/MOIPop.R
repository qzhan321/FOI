      ...
      for (k in 1:length(layers)) {
        ...
        for (nRealization in 1:nRealizations) {
          ...
          sampled_hosts_sub <- left_join(sampled_hosts, hosts, by = c("host_id")) %>% 
            ...
            select(host_id) %>% mutate(MOI = 0) # not the best way because not all individuals' MOI are 0, but will filter and keep the ones with MOI = 0 in the following step so in the end it is fine.  
