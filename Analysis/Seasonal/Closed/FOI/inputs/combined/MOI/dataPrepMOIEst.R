      ...
      for (j in 1:length(layers)) {
        ...
        if (seasonality == "seasonal") {
          if (layer < preIRS*T_YEAR & layer %% T_YEAR == 300) {
            p_detection <- (1 - 0.575)*0.75*0.711329
          } else if (layer %% T_YEAR == 180) {
            p_detection <- (1 - 0.115)*0.75*0.711329
          } else if (layer > preIRS*T_YEAR & layer %% T_YEAR == 300) {
            p_detection <- (1 - 0.27)*0.75*0.711329
          }
        } else {
        ...
        seed.id <- (i*100000 + j)*nRealization 
        set.seed(seed.id) 
        ...
        MOIInput <- infStrainDfSub %>% filter(host_id %in% posHostsDetected) %>% group_by(host_id) %>% summarise(DBLa_upsBC_rep_size = n_distinct(gene_id))
