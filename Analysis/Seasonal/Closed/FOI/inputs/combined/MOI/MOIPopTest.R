    ...
    for (j in 1:length(layersList)) {
      ...
      for (k in 1:length(layers)) {
        ...
        for (nRealization in 1:nRealizations) {
          ...
          for (nRealizationFurther in 1:nRealizations) {
            set.seed(seed.id)
            print(seed.id)
            seed.id <- seed.id + 1
            ...
            sampled_hosts_sub2 <- sampled_hosts_sub %>% filter(host_id %in% hostsNoInf)
            colnames(sampled_hosts_sub2)[1] <- "HostID"
            
            MOI <- do.call("bind_rows", list(MOIPosDetected, MOIPosMissedImputed, sampled_hosts_sub2)) %>% mutate(time = layer, nRealization = nRealization, nRealizationFurther = nRealizationFurther)
      ...
      write.csv(MOIAllFinal, file = paste0(saveDir4, "r", r, "_", layersLabel, "_", ageGroupLabel, "_test.csv"), row.names = F)
    }
    dbDisconnect(db)
  }
}
