...
saveDir0 <- paste0("/project2/pascualmm/QZ/PhD/projects/FOI/", folder, "/round", roundIndex, "/files/", arrivalType, "/FOI/inputs/MOIPopSingleTimePoint/")
  ...
  for (r in reps) {
    ...
    for (j in 1:length(layersList)) {
      ...
      for (k in 1:length(layers)) {
        ...
        MOIAll <- dfAll %>% filter(time %in% layer, age <= ageCutoff)
