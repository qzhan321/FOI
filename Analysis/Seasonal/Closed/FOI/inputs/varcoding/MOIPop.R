...
saveDir0 <- paste0("/project2/pascualmm/QZ/PhD/projects/FOI/", folder, "/round", roundIndex, "/files/", arrivalType, "/FOI/inputs/MOIPopVarcoding/")
  ...
  for (r in reps) {
    for (j in 1:length(layersList)) {
      ...
      for (nRealization in 1:nRealizations) {
        ...
        MOIAll <- bind_rows(MOIAll, dfAll %>% filter(time %in% layers, age <= ageCutoff))
      }
      ...
      write.csv(MOIAllFinal, file = paste0(saveDir3, "r", r, "_", layersLabel, "_", ageGroupLabel, ".csv"), row.names = F)
    }
  }
}
