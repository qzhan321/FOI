...
for (a in 3:3) {
  ...
  for (rep in reps) {
    ...
    for (d in ds) {
      ...
      MOIs1 <- MOIsAll %>% filter(time %in% times, age <= ageCutoff)
      MOIs2 <- MOIs1 %>% group_by(time) %>% summarise(n_total = sum(p)) 
      MOIs3 <- MOIs1 %>% group_by(time, MOI) %>% summarise(n = sum(p)) 
      MOIs <- MOIs3 %>% left_join(MOIs2, by = c("time"))
      stopifnot(nrow(MOIs)==(c+1)*length(times))
      print(unique(MOIs$n_total))
      
      MOI_temp <- NULL
      for (time_index in 1:length(times)) {
        time_temp <- times[time_index]
        MOIsSub <- MOIs %>% filter(time == time_temp)
        stopifnot(nrow(MOIsSub)==c+1)
        stopifnot(abs(sum(round(MOIsSub$n)) - unique(MOIsSub$n_total)) <= 2)
        MOI_temp_single <- rep(MOIsSub$MOI, round(MOIsSub$n))
        MOI_temp <- c(MOI_temp, MOI_temp_single)
      }
