...
calc_mu_i <- function(i = NA, a = NA, b = NA, c = NA, r = NA, a_R = NA, b_R = NA) {
  ...
  # a_R: time-averaging quantity for the average residual interarrival time at the departure instant of a customer who leaves behind n customers in the system (0 <= n <= c + r)
  # b_R: time-averaging quantity for the average residual service time of a randomly chosen busy server at the arrival instant of a customer who finds (leaves behind) n customers in the system (1 <= n <= c + r).
  # i between 1 and c+r
  if (i >= 1 & i <= c) {
    temp <- b - i * (a - a_R)
  } else if (i >= c + 1 & i <= c + r) {
...
calc_lambda_i <- function(i = NA, a = NA, b = NA, c = NA, r = NA, a_R = NA, b_R = NA) {
  # c servers + r waiting place: 0 to c + r - 1
  # a: expectation of interarrival times
  # b: expectation of service times
  # a_R: time-averaging quantity for the average residual interarrival time at the departure instant of a customer who leaves behind n customers in the system (0 <= n <= c + r)
  # b_R: time-averaging quantity for the average residual service time of a randomly chosen busy server at the arrival instant of a customer who finds (leaves behind) n customers in the system (1 <= n <= c + r)
...
calc_gamma_i <- function(i = NA, lambda = NA, a = NA, a_R = NA, mu_i_all = NA, lambda_i_all = NA, c = NA, r = NA) {
  ...
  # a_R: time-averaging quantity for the average residual interarrival time at the departure instant of a customer who leaves behind n customers in the system (0 <= n <= c + r)
  # mu_i_all: a vector storing values for all individual mu 
  # lambda_i_all: a vector storing values for all individual lambda. Note: those lambda_i are different from the lambda parameter for arrivals
  if (i == 0) {
    temp <- lambda * a_R 
  } else if (i >= 1 & i <= c + r - 1) {
...
estQLDist <- function(Va, ma, Vs, ms, c, r) {
  ...
  # Va: variance of the interarrival times
  # ma: mean of the interarrival times
  # Vs: variance of the service times
  # ms: mean of the service times
  a <- ma
  b <- ms 
...
calcNegLogLikelihood <- function(p, MOI) {
  minp <- min(p)
  if (any(p < 0)) {
    # print(minp)
    p[p <= 0] <- 0
  ...
  negLogLikelihood <- sum(-log10(p[MOI + 1]))
  return(list("negLogLikelihood" = negLogLikelihood, "minp" = minp))
}
