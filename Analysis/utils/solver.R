...
lh_nlogT <- function(p, MOI, epsilon) {
  if (length(p[p<=0]) == 0) {
    lh_nlogT <- sum(-log10(p[MOI]))
    return(lh_nlogT)
  } else if (length(p[p<=0]) > 0 & abs(min(p[p<=0])) < epsilon) {
    p[p<=0] <- p[p<=0] + epsilon
    p <- p/sum(p)
    lh_nlogT <- sum(-log10(p[MOI]))
    return(lh_nlogT)
    ...
    return(Inf)
    print("the negative probability is larger than epsilon!!!")
  }
}
