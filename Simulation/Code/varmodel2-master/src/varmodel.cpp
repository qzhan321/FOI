...
sqlite3_stmt * all_sampled_hosts_stmt;
...
void write_all_sampled_hosts(Host * host);
...
void recombine_infection(Infection * infection, bool recombLoad);
...
*** Initialization function implementations ***
    ...
    void validate_and_load_parameters() {
        ...
        if (ECTOPIC_RECOMBINATION_CREATE_NEW_ALLELE) {
            assert(P_ECTOPIC_RECOMBINATION_CREATE_NEW_ALLELE > 0.0 && P_ECTOPIC_RECOMBINATION_CREATE_NEW_ALLELE <= 1.0);
        }
...
*** Sample database output ***
    ...
    void initialize_sample_db() {
        ...
        sqlite3_exec(sample_db,
                     "CREATE TABLE IF NOT EXISTS all_sampled_hosts (time REAL, host_id INTEGER);",
                     NULL, NULL, NULL
        );
        ...
        sqlite3_exec(sample_db,
                     ...
                     "time REAL, pop_id INTEGER, n_infections INTEGER, n_infected INTEGER, n_infected_bites INTEGER, n_total_bites INTEGER, n_circulating_strains INTEGER, n_circulating_genes INTEGER, exec_time REAL, n_infected_bites_with_space INTEGER, FOI INTEGER"
                     ");",
                     NULL, NULL, NULL
        );
        ...
        sqlite3_exec(sample_db,
                     "CREATE TABLE IF NOT EXISTS sampled_duration (time REAL, duration REAL, host_id INTEGER, pop_id INTEGER, infection_count INTEGER, host_current_MOI INTEGER, infection_id INTEGER, strain_id INTEGER);",
                     NULL, NULL, NULL
        );
        ...
        sqlite3_prepare_v2(sample_db, "INSERT OR IGNORE INTO all_sampled_hosts VALUES (?,?);",-1, &all_sampled_hosts_stmt, NULL);
...
void finalize_sample_db() {
    ...
    sqlite3_finalize(all_sampled_hosts_stmt);
...
void recombine_infection(Infection * infection, bool recombLoad) {
    ...
    if (ECTOPIC_RECOMBINATION_CREATE_NEW_ALLELE) {
        ...
        // Otherwise, recombine the genes to produce one (under conversion) or two (if normal recombination) new genes
            ...
            Gene * rec_gene_1 = get_or_create_gene(rec_alleles_1, SOURCE_RECOMBINATION, draw_bernoulli(similarity*(double)src_gene_1->is_functional),false);
            ...
            // Under normal combination, both genes have material swapped
                ...
                Gene * rec_gene_2 = get_or_create_gene(rec_alleles_2, SOURCE_RECOMBINATION, draw_bernoulli(similarity*(double)src_gene_2->is_functional),false);
...
Gene * recombine_alleles(Gene * gene1, Gene * gene2, uint64_t breakpoint, double similarity, bool create_new_allele) {
    ...
    for(uint64_t i = 0; i < N_LOCI; i++) {
        if(i < breakpoint) {
            ...
            if((create_new_allele)&&(a1[i]!=a2[i])){
                ...
                // Add new AlleleRef for this allele
                AlleleRef * newAlRef = allele_ref_manager.create();
                newAlRef->locus = i;
                newAlRef->allele = n_alleles[i] - 1;
                allele_refs[i].push_back(newAlRef);
            }else{
                arc[i] = a2[i];
            }
        }
    ...
    RETURN(get_or_create_gene(arc, SOURCE_RECOMBINATION, draw_bernoulli(similarity*(double)gene1->is_functional), false));
}
        ...
        if ((MAX_ACTIVE_MOI-get_active_infection_count(host))>0) {
            ...
            if(SELECTION_MODE == SPECIFIC_IMMUNITY) {
                while(infection->expression_index < N_GENES_PER_STRAIN){
                    if(!(infection->expression_order[infection->expression_index]->is_functional)){
...
void clear_infection(Infection * infection, bool record) {
    ...
    if(record){
        if (now >= WRITE_DURATION_START) {
            if (host->population->total_cleared_infections%TOTAL_CLEARED_INFECTIONS_UNIT == 0) {
...
void update_biting_rate_change(Population * pop){
    ...
    if (pop->within_IRS_id == IRS_BITING_RATE_FACTORS[pop->current_IRS_id].size()) {
        ...
        pop->IRS_biting_rate = IRS_BITING_RATE_FACTORS[pop->current_IRS_id][pop->within_IRS_id];
        pop->IRS_immigration_rate_factor = IRS_IMMIGRATION_RATE_FACTORS[pop->current_IRS_id];
        pop->next_IRS_rate_change_time += IRS_DURATION;
        IRS_queue.update(pop);
        pop->within_IRS_id++;
    }
...
void do_recombination_event() {
    ...
    recombine_infection(infection, RECOMBINATION_LOAD);
    ...
    void sample_hosts() {
        ...
        for(Host * host : host_manager.objects()) {
            if(draw_bernoulli(double(HOST_SAMPLE_SIZE)/double(host_manager.size()))){
                write_all_sampled_hosts(host);
                write_sampled_immunity(host);
                
                for(Infection * infection : host->infections) {
                    if (infection->expression_index >= 0) {
                        if(MICROSCOPY_SAMPLING_ON) {
                            if(draw_bernoulli(P_MICROSCOPY)) {
...
void write_all_sampled_hosts(Host * host) {
    sqlite3_bind_double(all_sampled_hosts_stmt, 1, now);
    sqlite3_bind_int64(all_sampled_hosts_stmt, 2, host->id);
    
    sqlite3_step(all_sampled_hosts_stmt);
    sqlite3_reset(all_sampled_hosts_stmt);
}
...
void write_duration(Host * host, Infection * infection) {
    ...
    sqlite3_bind_int64(sampled_duration_stmt, 7, infection->id);
    sqlite3_bind_int64(sampled_duration_stmt, 8, infection->strain->id);
...
void update_global_mutation_time() {
    BEGIN();
    double mutRate = ECTOPIC_RECOMBINATION_RATE * N_GENES_PER_STRAIN * (N_GENES_PER_STRAIN - 1) / 2.0 * current_pop_size * P_ECTOPIC_RECOMBINATION_CREATE_NEW_ALLELE * P_GENE_INVASION * REGION_TO_LOCAL_POP_SIZE_RATIO;
